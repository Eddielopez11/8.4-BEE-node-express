...
export default React.createClass({
  ...
  creditCardValidator() {
    var numberValidated = this.state.cardValidate.split(" ")
    var dateValidated = this.state.dateValidate.split(" ")
    var cvvValidated = this.state.cvvValidate.split(" ")
    var nameValidated = this.state.nameValidate.split(" ")
    if(numberValidated.length === 2 && dateValidated.length === 2 && cvvValidated.length === 2 && nameValidated.length === 2) {
      this.setState({
        creditCard: "creditCard"
      })
    } else {
      this.setState({
        creditCard: "creditCard creditCard--incomplete"
      })
    }
  },
  render() {
    return (
      <section>
        <div className={this.state.creditCard}>
          <h2 className="creditCard__bank">{"Bank of Frosty"}</h2>
          <form action="#" method="POST" onKeyUp={this.creditCardValidator}>
            <input type="text"
                   name="number"
